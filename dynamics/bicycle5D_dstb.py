...
class BicycleDstb5D(BaseDstbDynamics):
  ...
  def integrate_forward_jax(
    ...
    def crit_delta(args):
      ...
      def crit_delta_vel(args):
        ...
        def vel_then_delta(args):
          state_tmp1 = self._integrate_forward_dt(
              state, ctrl_clip, dstb_clip, c_vel
          )
          state_tmp2 = self._integrate_forward_dt(
              state_tmp1, jnp.array([0., ctrl_clip[1]]), dstb_clip,
              c_delta - c_vel
          )
          return self._integrate_forward_dt(
              state_tmp2, jnp.zeros(2), dstb_clip, self.dt - c_delta
          )

        def delta_then_vel(args):
          state_tmp1 = self._integrate_forward_dt(
              state, ctrl_clip, dstb_clip, c_delta
          )
          state_tmp2 = self._integrate_forward_dt(
              state_tmp1, jnp.array([ctrl_clip[0], 0.]), dstb_clip,
              c_vel - c_delta
          )
          return self._integrate_forward_dt(
              state_tmp2, jnp.zeros(2), dstb_clip, self.dt - c_vel
          )
      ...
      def crit_delta_only(args):
        state_tmp = self._integrate_forward_dt(
            state, ctrl_clip, dstb_clip, c_delta
        )
        return self._integrate_forward_dt(
            state_tmp, jnp.array([ctrl_clip[0], 0.]), dstb_clip,
            self.dt - c_delta
        )
    ...
    def non_crit_delta(args):
      ...
      def crit_vel_only(args):
        ...
        return self._integrate_forward_dt(
            state_tmp, jnp.array([0., ctrl_clip[1]]), dstb_clip,
            self.dt - c_vel
        )
  ...
  def disc_deriv(
    ...
    deriv = deriv.at[3].set(
        state[2] * jnp.tan(state[4]) / self.wheelbase + disturbance[3]
    )
